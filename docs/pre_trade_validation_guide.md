...
### 5. 데이터 부족 시 대체 플로우 (신규)
- 거래 횟수가 기준 미달(기본 2회)인 경우 세 가지 옵션 중 하나를 적용해 리스크를 낮춘다.
  1. **포지션 축소**: 초기 진입 비중을 50%로 제한하고 추가 진입을 금지한다.
  2. **상위 타임프레임 보조 검증**: 30분봉 또는 일봉 기준으로 동일 전략을 재시뮬레이션해 승률·PF가 기준을 충족하면 진입 허용.
  3. **감시 리스트 유지**: 즉시 진입을 포기하고 다음 3개 캔들까지 데이터를 축적한 뒤 재검증.
- 위 조건이 모두 충족되지 않으면 `PreTradeValidator`가 `reason="INSUFFICIENT_SAMPLE"` 로거를 남기고 RiskManager에 주의 플래그를 기록한다.
...
validator = PreTradeValidator(
    ...
    min_avg_profit=0.5,    # 최소 평균 수익률 (%), 데이터 부족 시 30분봉 재검증
...
> ℹ️ **RiskManager 연동**  
> - 주간 손실이 -3%에 도달했거나 연속 손실 3건이 발생하면, 위 기준을 자동으로 강화(승률 ≥ 60%, PF ≥ 1.5)하도록 `AdaptiveValidator` 설정을 변경한다.  
> - 강화 모드 해제 조건은 일일 손실 -2% 이내, 주간 손실 -1% 이내 복귀다.
...
## ⚠️ 데이터 품질 유의사항
- 실시간 5분봉 데이터가 누락되면 `historical_data`가 Mock 값으로 대체될 수 있으므로, `stats['is_mock']` 플래그가 True일 때는 **무조건 진입 금지** 후 재검증한다.
- 고저가 스플릿, 상한가/하한가 구간 등 특수 케이스는 Profit Factor가 과도하게 높게 나올 수 있으므로 최소 거래 횟수를 3으로 상향하거나 직접 차트를 확인한다.
- 검증 결과는 `logs/pre_trade_validator_*.log`에 저장되며, RiskManager 경고 플래그와 함께 주간 리스크 리포트에 집계된다.
